...
//Create player and initialise starting values
void init( void )
{
...
void update( void )
{
    ...
    if( velY >= 0 )
    {
        // Check if player has hit the ground or a platform while falling
        onPlatform = false;
    ...
    if( onPlatform )
    {
        ...
        else if( velX != -platformSpeed && animation != MOVE )
        {
            // If starting to move, set animation to move
            animation = MOVE;
            currentFrame = 0;
            totalFrames = faceRight ? WALKRIGHT_FRAMES : WALKLEFT_FRAMES;
        }
        else if( velX == -platformSpeed && velY == 0 && animation != IDLE )
        {
            // If not moving
            animation = IDLE;
            currentFrame = 0;
...
void draw( void )
{
    ...
    redrawBackground( (prevX - PLAYER_WIDTH / 2) / 1000,
                      (prevY - PLAYER_HEIGHT / 2) / 1000,
                      PLAYER_WIDTH / 1000,
                      PLAYER_HEIGHT / 1000,
                      background );
    ...
    for( int i = 0; i < NUM_PLATFORMS; i++ )
    {
        ...
        if( allPlatforms[ i ].prevX - PLATFORM_WIDTH / 2 <= 240000 ||
            allPlatforms[ i ].prevX + PLATFORM_WIDTH / 2 >= 0 )
        {
            redrawBackground( (allPlatforms[ i ].prevX - PLATFORM_WIDTH / 2) / 1000,
                              (allPlatforms[ i ].y - PLATFORM_HEIGHT / 2) / 1000,
                              PLATFORM_WIDTH / 1000,
        ...
        {
            drawImage(( allPlatforms[ i ].x - PLATFORM_WIDTH / 2) / 1000,
                      ( allPlatforms[ i ].y - PLATFORM_HEIGHT / 2) / 1000,
                       PLATFORM_WIDTH / 1000,
                       PLATFORM_HEIGHT / 1000,
                       platform );
    ...
    if( onPlatform )
    {
        ...
        {
            drawAnimation( (playerX - PLAYER_WIDTH / 2) / 1000,
                           (playerY - PLAYER_HEIGHT / 2) / 1000,
                           PLAYER_WIDTH / 1000, PLAYER_HEIGHT / 1000,
                           currentFrame,
                           totalFrames,
                           faceRight ? idleRight : idleLeft );
        }
        else
        {
            drawAnimation( (playerX - PLAYER_WIDTH / 2) / 1000,
    ...
    {
        drawImageTrans( (playerX - PLAYER_WIDTH / 2) / 1000,
                        (playerY - PLAYER_HEIGHT / 2) / 1000,
                        PLAYER_WIDTH / 1000,
                        PLAYER_HEIGHT / 1000,
                        faceRight ? jumpRight : jumpLeft );
    }
}
